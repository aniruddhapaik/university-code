...
class EvaluateExpression {
 private:
  ...
  double evaluatePostfix() {
    ...
    while (counter < this->expression_size) {
      ...
      if (*current_char == ' ') {
      ...
      } else if (static_cast<int>(*current_char) > 47 and
                     static_cast<int>(*current_char) < 58 or
                 *current_char == '.') {
        ...
        if (this->operands_stack.size() < 2) {
          std::cout << "Something is wrong with the expression!\n"
                    ...
                    << "-> ratio should be operands (n):(n-1) operators"
                    << std::endl;
          return -1;
        } else {
      ...
      if (*current_char == ' ') {
        if (not temp_term.empty()) {
          ...
          temp_term = "";
                 ...
                 *current_char == '.') {
        /*
        in the else if statement above I am making sure that every character
        read, that is not an operand, is between ASCII value ]47, 58[, which are
        ASCII values for characters 0 to 9 or the current character is a '.'
        which means there is a rational number in the expression that we need to
        read correctly.
        */
        // evaluates to true if current_char is not an operator.
        temp_term.push_back(*current_char);
 ...
 public:
  ...
  void loadPostfixTestStrings(std::string& expression) {
    ...
    while (not this->operands_stack.empty()) {
      this->operands_stack.pop();
    }
