...
class KnapSack {
 public:
  void getItemList(float maxcap, std::vector<std::pair<float, float>>& itlist) {
    ...
    std::cout << "After Sorting according to ratio | Ascending Profit\n";
    this->printList(this->sortedlist);
 ...
 private:
  ...
  const void printList(std::vector<Item*>& list) {
    ...
    std::cout << "------------------------------------" << std::endl;
    for (auto& item : list) {
      std::cout << '[' << item->itemnumber << ": " << item->weight << '|'
                << item->profit << "] ";
    }
    std::cout << '\n' << std::endl;
  }
  ...
  void fillBag() {
    ...
    while (maxcap > 0) {
      if ((maxcap - this->sortedlist.back()->weight) >= 0) {
        ...
        this->totalprofit +=
            this->sortedlist.back()->profit * takefractionofprofit;

        this->sortedlist.back()->toinclude = true;

        std::ostringstream ossr;
        ossr << std::fixed << std::setprecision(2)
             << maxcap / this->sortedlist.back()->weight;
