...
const void SimpleGraph::getEdgeList(Direction dir,
                                    std::vector<edgecost>& edgelist) {
  this->direction = dir;
  this->buildGraph(edgelist);
}

const void SimpleGraph::setSource(unsigned int src) {
...
const void SimpleGraph::printVerticesInfo() const {
  std::cout << "============================================" << std::endl;
  for (auto& pair : this->allnodes) {
    std::cout << "Vertex " << pair.second->label << std::endl;

    if (pair.second->cost_from_source ==
    ...
    if (pair.second->path_from_source.size()) {
      for (std::vector<unsigned int>::iterator it =
               pair.second->path_from_source.begin();
           it != pair.second->path_from_source.end() - 1; ++it) {
        std::cout << *it << " -> ";
      }
...
void SimpleGraph::buildGraph(std::vector<edgecost>& edgelist) {
  for (edgecost& pair : edgelist) {
    ...
    this->allnodes.find(pair.first.first)
        ->second->linksandcost.emplace(pair.first.second, pair.second);

    if (this->direction == Direction::undirected) {
      // Setting vertex 2's neighbor: vertex 1 (if the graph is undirected)
      this->allnodes.find(pair.first.second)
          ->second->linksandcost.emplace(pair.first.first, pair.second);
    }
  }
}

std::ostream& operator<<(std::ostream& COUT, const SimpleGraph& graph) {
