...
class SinglyLinkedList {
 private:
  ...
  void popNodeAfter(Node* parent) {
    // expects the parent node of the node the user wants to delete.
 ...
 public:
  ...
  void popFront() {
    if (this->isEmpty()) {
      ...
      /*
      the above if statement is for the case there was only one node
      in the list before the deletion,
      so assigning the tail to nullptr as well
      since the head now points to nullptr.
      */
    }
  }
  ...
  void popBack() {
    if (this->isEmpty()) {
      ...
      // this block runs if there was only one node,
      // so popBack() would render the list empty.
      delete this->Head;  // freeing memory
      this->listsize--;
      this->Head = nullptr;
      this->Tail = nullptr;
      ...
      while (temp->next->next != nullptr) {
        // this runs if there are at least 2 nodes
  ...
  void addBefore() {
    if (this->isEmpty()) {
      ...
      this->pushFront();
      // since list is empty, addAfter() automatically switches to pushing to
      // the list, so now pushFront() or pushBack() would result in doing the
      // same thing.
    } else {
      if (this->listsize == 1) {  // checking if only one node exists
        std::cout << "Only one node exists.\n-> Adding new head.." << std::endl;
        this->pushFront();
        // since there is only one node, adding before would mean
        // you want to add to the before the only node there is.
        ...
        if (this->handleInput()) {
          if (this->Head->key == this->last_input) {
            // even though there are more than 1 node, in case you still
            // want to add before the first node...
            // ...addBefore() automatically switches to pushFront() function
            this->pushFront();
            return;
          } else {
            // executes when you want to add somewhere after the first head
            // node.
            Node* node_parent = this->findNodeBefore(this->last_input);
            // suppose you want to add to a certain node,
            // findNodeBefore() gets the parent node of
            // the node before which you want to add a new node
            if (node_parent == nullptr) {
              return;
            }
            // if the node requested for is not found,
            // the above evaluates to true
            else {
              // getting user to enter new key to actually
              // create a node before requested node
              std::cout << "Enter key to add before " << this->last_input
                        << ": " << std::flush;
              if (this->handleInput()) {
                addNodeAfter(node_parent);
              }
              return;
            }
          }
  ...
  void addAfter() {
    if (this->isEmpty()) {
      ...
      this->pushBack();
      // since list is empty, addAfter() automatically switches to pushing to
      // the list. pushFront() or pushBack() would result in doing the same
      // thing.
    } else {
      if (this->listsize == 1) {
        std::cout << "Only one node exists.\nAdding a new tail.." << std::endl;
        this->pushBack();
        // since there is only one node, choosing to addAfter can
        // only mean you are want to add to the end of the list.
        return;
      } else {
        ...
        if (this->handleInput()) {
          ...
          if (this->Tail->key == this->last_input) {
            ...
            // the function findNodeBefore(int)
            // does not account for the head node, so
            std::cout << "Enter key to add after " << this->last_input << ": "
                      << std::flush;
            if (this->handleInput()) {
              this->addNodeAfter(this->Head);
            }
            return;
            // I don't need to check if the head->next is a nullptr or not,
            //   it is guaranteed that the listsize is > 1,
            //   so the head->next is definitely not nullptr.
          } else {
            // Although findNodeBefore returns the node before the
            // node of interest, we can just move to the next node,
            // after which we can add a node after it.
            // It is guaranteed that the node after which the user wants to
            // add is not the current tail node,
            // because that case has already been handled
            Node* parent = this->findNodeBefore(this->last_input);
            if (parent == nullptr) {
              return;
            }  // checking node ...
               // requested for exists or not
            else {
              std::cout << "Enter key to add after " << this->last_input << ": "
                        << std::flush;
              if (this->handleInput()) {
                addNodeAfter(parent->next);
              }
              return;
            }
          }
  ...
  Node* findNode() {
    ...
    if (this->handleInput()) {
      Node* temp = this->Head;
      // checking every node if its value matches the key the user is looking
      // for
      while (temp != nullptr) {
        // evaluates to false once the tail->next is reached, since tail->next
        // is nullptr
        if (temp->key == this->last_input) {
          std::cout << "Key " << this->last_input << " has been found!"
                    << std::endl;
          return temp;
        }
        temp = temp->next;
      ...
      if (this->listsize == 1) {
        ...
        if (this->handleInput()) {
          if (this->last_input == 1) {
            ...
            std::cout
                << "You have chosen to not delete the only key there is\n";
  ...
  int peekIndex() {
    if (this->isEmpty()) {
      ...
      if (this->listsize > 1) {
        ...
        std::cout << "\nEnter a number in the range of [0, "
                  << this->listsize - 1 << "]";
        ...
        if (this->handleInput()) {
          if (this->last_input >= 0 and this->last_input < this->listsize) {
            ...
            std::cout << "Key at index " << this->last_input << " is "
                      << temp->key << std::endl;
            return temp->key;
          } else {
        ...
        std::cout << "The only key in the list is " << this->Head->key
                  << std::endl;
        return this->Head->key;
      }
    }
  ...
  void delIndex() {
    if (this->isEmpty()) {
      ...
      if (listsize > 1) {
        ...
        std::cout << "\nEnter a number in the range of [0, "
                  << this->listsize - 1 << "]";
        std::cout << "\nEnter index at which key is to be deleted: "
                  << std::flush;
        if (this->handleInput()) {  // enter index of node to deleted
          ...
          if (this->last_input > 0 and this->last_input < this->listsize) {
            ...
            // negating 1 purposely to first get to the parent of the node the
        ...
        // if there is only one node, delIndex() would mean deleting the only
        // node, so doing popBack(). popFront() would work here too since there
        // is only one node;
        std::cout
            << "Do you want to delete the only key there is in the list?\n";
        std::cout << "(1 = yes, (any integer except 1) = NO)(Default is NO): "
                  << std::flush;
        this->last_input = 0;  // purposely setting it to 0, since default is NO
        if (this->handleInput()) {
          if (this->last_input == 1) {
