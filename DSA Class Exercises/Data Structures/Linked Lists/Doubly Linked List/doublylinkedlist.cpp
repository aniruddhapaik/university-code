...
class DoublyLinkedList {
 private:
  ...
  void addNodeBefore(Node* node) {
    Node* prev = node->prev;
    // creating and assigning new node's address to the next pointer of the
    // previous node; this previous node is the previous node of the current
    // node before the addition of the new node in between them
    prev->next = new Node(this->last_input, prev, node);
    // assigning the appropriate address to the current node's previous node
    // pointer...
  ...
  void popNode(Node* node) {  
    // expext the parent node of the node the user
    // wants to delete.
    // this function does not account for the case when the argument node is the
    // head or tail node
 ...
 public:
  ...
  void pushBack() {
    ...
    if (this->handleInput()) {
      if (this->Tail == nullptr) {
        ...
        this->Tail->next = new Node(this->last_input, this->Tail,
                                    nullptr);  // key, previous, next
  ...
  void popFront() {  // check if this func account for head = tail
    if (this->isEmpty()) {
      ...
      // not to forget to set the previous node ptr to nullptr! since this
      // node is the new head.
      // the above if statement is for the case there was only one node in the
      // list before the deletion, so assigning the tail to nullptr as well
  ...
  void popBack() {  // checking
    if (this->isEmpty()) {
      ...
      // this block runs if there was only one node, so popBack() would render
      // the list empty.
      delete this->Head;  // freeing memory
      this->listsize--;
      this->Head = nullptr;
      ...
      Node* newtail =
          this->Tail->prev;  // first saving the previous node before the tail
      ...
      newtail->next = nullptr;  // making sure the new tail's next pointer is
                                // not a dangling pointer; setting it to nullptr
  ...
  Node* findNode() {
    ...
    if (this->handleInput()) {
      ...
      // for
      while (temp != nullptr) {  // evaluates to false once the tail->next is
                                 // reached, since tail->next is nullptr
  ...
  void addBefore() {
    if (this->isEmpty()) {
      ...
      this->pushFront();
      return;
      // since list is empty, addBefore() automatically switches to pushing a
      // new node to the list. pushFront() or pushBack() would result in doing
      // the same thing.
    } else {
      if (this->listsize == 1) {
        ...
        this->pushFront();
        return;
        // since there is only one node, choosing to addBefore can only mean you
        // are want to add to the beginning of the list.
      } else {
        std::cout << "Enter the key before which you add a new key.\n";
        Node* node = this->findNode();
        if (node == nullptr) {
          return;
        }  // checking node requested for exists or not
  ...
  void addAfter() {
    if (this->isEmpty()) {
      ...
      this->pushBack();
      return;
      // since list is empty, addAfter() automatically switches to pushing a new
      // node to the list. pushFront() or pushBack() would result in doing the
      // same thing.
    } else {
      if (this->listsize == 1) {
        ...
        this->pushBack();
        return;
        // since there is only one node, choosing to addAfter can only mean you
        // are want to add to the end of the list.
      } else {
        ...
        if (node == nullptr) {
          ...
          this->pushBack();
        }  // accounting for the case where the user wants to add after the tail
        else {
  ...
  void delKey() {
    if (this->isEmpty()) {
      ...
      if (this->listsize == 1) {
        ...
        if (this->handleInput()) {
          Node* node =
              this->findNode(this->last_input);  // using an overloaded function
          if (node == nullptr) {
            return;
          } else if (node == this->Head) {
            popFront();
