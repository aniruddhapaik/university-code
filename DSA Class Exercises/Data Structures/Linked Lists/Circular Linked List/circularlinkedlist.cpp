 ...
 private:
  ...
  Node* findNodeBefore() {  // it is guaranteed the listsize is atleast 2
    ...
    while (temp->next->next != this->headnode) {
      if (this->last_input == temp->next->value) {
        return temp;
      }
      temp = temp->next;
    }
  ...
  void popFront() {
    if (this->listsize == 0) {
      ...
      if (this->listsize == 1) {
        ...
        this->tailnode->next =
            this->headnode;  // linking the new head node after the tail node
 ...
 public:
  ...
  int pushAfter() {
    if (this->listsize == 0) {
      ...
      std::cout << "Enter after which node value you want to push new node: "
                << std::flush;
      if (this->handleInput()) {
  ...
  void deleteKey() {
    if (this->listsize == 0) {
      ...
      if (this->listsize == 1) {  // since only one node is left, deleting that
        std::cout
            << "Delete the only node left?\n(1 = yes, 0 = no; no is default): "
            << std::flush;
        if (this->handleInput()) {
  ...
  const void displayList() const {
    if (this->listsize == 0) {
      ...
      std::cout << temp->value << " | -> HEAD: " << temp->next->value
                << std::endl;  // the tail node value
    }
  }
};