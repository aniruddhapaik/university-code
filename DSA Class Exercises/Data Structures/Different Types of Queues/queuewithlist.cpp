...
class Queue {
 ...
 public:
  ...
  int dequeue() {
    if (this->queuesize) {
      std::cout << "Dequeueing first element: " << this->head->value
                << std::endl;
  ...
  int lastElement() {
    if (this->queuesize) {
      std::cout << "The last element in the Queue is " << this->tail->value
                << std::endl;
  ...
  int peek() {
    if (this->queuesize) {
      std::cout << "There are " << this->queuesize << " element in the queue."
                << std::endl;
      if (this->queuesize > 1) {
        std::cout << "Enter index between 0 and " << (this->queuesize - 1)
                  << ".\n";
      } else {
        std::cout << "The only index available to peek at is 0.\n";
      }
      if (this->getInput()) {
        int counter = 0;
        Node* temp = this->head;
        if (this->last_input >= 0 and this->last_input < this->queuesize) {
          ...
          std::cout << "The element at index " << this->last_input << " is "
                    << temp->value << std::endl;
          return temp->value;
        } else {
