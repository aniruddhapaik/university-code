...
class PriorityQueueWithArray {
 private:
  ...
  void push(Node* temp_node) {
    if (this->queue->empty()) {
      this->queue->push_back(*temp_node);
    } else {
      size_t counter{0};
      while (counter < this->queue->size()) {
        if (temp_node->priority < this->queue->at(counter).priority) {
          counter++;
 ...
 public:
  ...
  void enQueue() {
    ...
    if (this->handleInput()) {
      temp_element = this->last_input;
    } else {
      return;
    }

    std::cout << "Enter priority for element (" << temp_element
              << ") only: " << std::flush;
