...
class Sender { 
  ...
  public void sendFrame(Receiver receiver, int totalFrames) { 
    ...
    while (framesSent < totalFrames) { 
      System.out.println("Sender: Sending frame with sequence number " + sequenceNumber);
      ...
      if (frameLost) { 
        System.out.println("[!] Sender: Frame lost in transmission. Resending frame...\n");
      } else { 
...
class Receiver { 
  ...
  public void receiveFrame(int sequenceNumber, boolean frameCorrupted) { 
    ...
    if (sequenceNumber == expectedSequenceNumber) { 
      System.out.println("Receiver: Frame received successfully with sequence number " + sequenceNumber);
